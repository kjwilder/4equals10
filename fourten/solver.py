...
def solve_one(digits, expected, operators):
    sq = "No solution"
    solution = try_to_solve(digits, expected=expected, operators=operators)
    if solution:
        sq = ": ".join(solution[::-1])
        print(f"{sq}")
        return
    ...
    for perm in itertools.permutations(digits):
        solution = try_to_solve(perm, expected=expected, operators=operators)
        if solution:
...
def main():
    ...
    if args.digits:
        ...
        solve_all(args.num_digits, args.expected, args.operators)
